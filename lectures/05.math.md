...
3. Extract the vector from the quaternion - $v_{result} = {(v_x^' v_y^' v_z^') \over {\varphi \over 2}}$
...
$$
q_{total} = q_3 q_2 q_1 \\\\
v' = q_{total} v q_{total}^{-1} = q_3 q_2 q_1 v q_1^{-1} q_2^{-1} q_3^{-1}
$$
...
point $\alpha$ lying some percentage away from the endpoints.

--- VERTICAL SLIDE ---

* Interpolation attempts to simulate an unknown or complex function
with a simple one - a line, a parabola, a cubic curve
