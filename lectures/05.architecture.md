...
- Adding a new entities is simple
    - Just add several components
    - Designer friendly

--- VERTICAL SLIDE ---

- Working with such entities is by talking to the components
...
- No longer need for separate function for dealing damage to players, npc, enemies, bosses, etc
...
public:
   ...
   {
       auto currentPos = myEntity->GetComponent<Transform>().position;
       myEntity->GetComponent<PhysicsComponent>().velocity += SomehowSteerTowards(currentPos, objectToOrbitAround);
   }
}
```
...
- No need for a programmer to create new c++ class.

--- VERTICAL SLIDE ---

Cons

- Access to components are through an indirection
    - Pointer
    - Search in entity
    - Virtual call (possibly)
...
What a program is doing on most basic level ?

--- VERTICAL SLIDE ---

Transforming one data from one format to another

For example - transforming player input to colors on the screen.
...
struct MoveComponent : public Component { 
    ...
    virtual void Start() override{ bounds = FindOfType<WorldBoundsComponent>(); }
...
Very used in graphics renderers and system programming.
Starting to gain more tracktion in gameplay programming as well.

--- VERTICAL SLIDE ---

Very big theme which a lot can be talked about.
Here is a [book](http://www.dataorienteddesign.com/dodmain/) about the topic.
...
void MoveSystemUpdate(EntityManager& manager) {
    for(auto&& [position, acceleration] : manager.QueryComponents<Position, Acceleration>()) {
        position.position += 
...
- Different that most programmers are used to
