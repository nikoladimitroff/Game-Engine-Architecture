...
What's are `const` refs and `const` pointers?
...
* Standard C++ doesn't necessarily give enough control / power
  * no matter the version of the language - '98, '03, '11, '14, '17
  * so sometimes we go back to basic C and move from there
...
* **no** `iostream`
  * hidden allocations -> low performance
  * hidden global lock for the locale -> low performance
* `FILE*` - the C way
...
* Use Google or suggested books to learn them

--- VERTICAL SLIDE ---

### Auto

Saves you from writing the variable type
...
* Whenever you access a resource
  * lock it in constructor
  * Unlock it in destructor

TBD: EVERYTHING BELOW THIS POINT
...
* Exceptions are frowned upon in many environments
  * forbidden on the previous generation of consoles (no runtime support)
...
### Quick review of how a computer works
