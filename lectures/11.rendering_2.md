...
--- VERTICAL SLIDE ---
  ...
  * Might be recorded in space (probe)
  * Materials
...
--- VERTICAL SLIDE ---
DIRECTX12 Synchronization FENCES
...
* Dynamic Objects: Transforms are updated every frame (characters)
...
struct Goblin : public DybamicObject;

struct GraphicWorld
{

  //data oriented by type, not one Object
  std::vector<mat44>    m_static_transforms;
  std::vector<mat44>    m_dynamic_transforms;
 ...
 * Objects may define 32 bits keys for sorting ( 24 bits for depth, materials)
...
--- VERTICAL SLIDE ---
Filtering and grouping (can happen on the CPU or GPU)
* Filter the lists somehow to make them smaller

* Group them together (example)
 * 64 bit key 3 bits per view, 10 bits per material, 16 bits for custom data
 * Iterate over objects, every object inserts keys one or more keys
 * Sort the array
 * Split the ranges

* Filtering is by view
 * Filter objects, parts of objects, triangles
 * Data oriented design is important here.

 --- VERTICAL SLIDE ---
 * We have lists of objects grouped by criteria
 * Now we have to submit them to the GPU with the API
 * Draw Call Commands

 --- VERTICAL SLIDE ---
 * Render Passes (Typical)
 * Depth Pass
 * GBuffer Pass
 * Shadows Pass
 * Reflection Pass ( water )
