...
Smart pointers are RAII helpers.

--- VERTICAL SLIDE ---

Every *smart pointer* implements
...
public:
    ...
    {
        std::cout << PrimaryWeapon->GetName(); // "Silver blade of overdone programming"
    }
}
```
...
std::unique_ptr<char[]> ReadFile(const char* path)
{
    ...
    const bool success = TryReadFile(path);
    if (!success)
    {
        // buffer is now nullptr
        buffer.reset();
    }
    return buffer;
}
