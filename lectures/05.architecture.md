...
void CheckBullet(Bullet& b, Player& p) {
    if(p.intersects(b.x, b.y, b.z)) {
...
void CheckBullets(Bullets& b, Player& p) {
    for(int i = 0; i < b.count; ++i) {
        if(p.intersects(b.Xs[i], b.Ys[i], b.Zs[i])) {
            p.applyDamage(b.damages[i]);
        }
    }
}
```
