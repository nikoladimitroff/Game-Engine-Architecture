...
- There are no bones actually in the skeleton
    - Techically, the bones are the edges between joints but that does not matter
...
system change from the joint to its parent.
...
struct AnimationClip
{
    ...
    float CurrentTime; // Could be local, could be global
    float PlaybackRate;
...
...I had to put a *skinny* cat for obvious reasons

![Skinny cat](resources/13.animation/skinny_cat.png) # TODO
...
$$
v\_j = v\_M^B B\_{M \to j} = B\_{j \to M}^{-1} v\_M^B \\\\
...
{
    // Note: WeightedAverage on matrices is tricky. See next section
...
The process of mixing multiple clips (poses)

![Blending cat](resources/13.animation/blending_cat.jpg) # TODO
...
<!-- .element class="fragment" data-fragment-index="0" --> *Insert grumpy cat here*
